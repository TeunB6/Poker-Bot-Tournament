...
class Blind(Enum):
    DEFAULT = auto() # No blind
    SMALL = auto() # Opens the pot with a bet of BIG_BLIND // 2
    BIG = auto() # Raises to BIG_BLIND 

class MoveType(Enum):
    RAISE = auto() # Move to raise the pot
    CALL = auto()   # Match the current bet
    CHECK = auto()  # Do not open betting
    FOLD = auto() # Exit the pot sets state to OUT
    
class PlayerState(Enum):
    OUT = auto() # Folded
    IN = auto() # Is in latest pot and has put in enough chips
    TO_CALL = auto() # Player is in the latest pot and needs to call a raise
    ALL_IN = auto() # Player is all-in and cannot take any further actions this hand
